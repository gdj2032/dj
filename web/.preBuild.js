...
function parseServiceExports() {
  const rootFolder = path.resolve('src/services');
  ...
  serviceFiles.forEach((name) => {
    ...
    if (segs[0] === 'index') {
      indexFile = path.resolve(rootFolder, fileName);
      return;
    }
  ...
  fs.writeFileSync(path.resolve(rootFolder, 'index.tsx'), fileContent, 'utf8');
}
function parseReduxExports() {
  const rootFolder = path.resolve('src/stores');
  const reduxFolders = fs.readdirSync(rootFolder);
  let fileContent = '// 此文件为编译时自动生成的代码，请勿更改, 改了也没有用' + os.EOL;
  let indexFile;
  const reducers = [];
  reduxFolders.forEach((name) => {
    const folderName = path.resolve(rootFolder, name);
    if (name.match(/^index\.(ts|tsx|js|jsx)$/)) {
      indexFile = folderName;
      return;
    }
  ...
  reducers.forEach(name => {
    fileContent += `import * as ${name}Action from './${name}';${os.EOL}`;
  })
  reducers.forEach(name => {
    fileContent += `export { ${name}Action };${os.EOL}`;
  })
  fileContent += `export * from './store';${os.EOL}`;
  fileContent += `export * from './hooks';${os.EOL}`;
...
function findCodeFile(folder, cache) {
  const absPath = path.resolve('src/pages', folder);
...
function getRoutePath(file) {
  const rs = new reallines(path.resolve('src/pages/', file));
  let line;
  let lineNum = 0;
  while (line = rs.next()) {
    lineNum++;
...
function makeRoute(route) {
  ...
  return `(${args}) => createPath('${route}', { ${args} })`
}
function parsePagePath() {
  const allFiles = [];
  // 第一层目录不管
  const rootFolder = path.resolve('src/pages');
  ...
  allFiles.forEach(f => {
    ...
    const pathInfo = /^(.*)\.(tsx|ts|jsx|js)$/.exec(f)[1];
  ...
  // 先比较有木有变化，如果没有，则不需要，否则这里写完文件，又会触发编译，会死循环
  const defFile = path.resolve(process.cwd(), 'src/pages/pageRoutes.tsx');
